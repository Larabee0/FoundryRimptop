    ...
    return class TargetedApplicationElement extends Base {
      ...
      set  targetingMode(mode) {
        ...
        if ( (mode === "targeted") && (this.selectedTokensHook !== null) ) {
          ...
          this.selectedTokensHook = Hooks.on("controlToken", foundry.utils.debounce(async () => await this.buildTargetsList(), 50));
        }
      }
      ...
      buildTargetContainer() {
        ...
        if ( !this.chatMessage?.getFlag("rimtop", "targets")?.length ) this.targetSourceControl.hidden = true;
